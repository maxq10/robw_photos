...
if ( ! class_exists( 'WpssoSubmenuStyles' ) && class_exists( 'WpssoAdmin' ) ) {

	class WpssoSubmenuStyles extends WpssoAdmin {
    ...
    .'.$lca.'-buttons
		...
		protected function add_meta_boxes() {
			...
			add_meta_box( $this->pagehook.'_styles',
				_x( 'Social Sharing Styles', 'metabox title', 'wpsso' ),
					array( &$this, 'show_metabox_styles' ), $this->pagehook, 'normal' );
		}

		public function show_metabox_styles() {

			if ( file_exists( WpssoSharing::$sharing_css_file ) &&
				( $fsize = filesize( WpssoSharing::$sharing_css_file ) ) !== false )
					$css_min_msg = ' <a href="'.WpssoSharing::$sharing_css_url.'">minimized css is '.$fsize.' bytes</a>';
			else $css_min_msg = '';

			$this->p->util->do_table_rows( 
				array( 
					$this->form->get_th_html( _x( 'Use the Social Stylesheet',
						'option label', 'wpsso' ), null, 'buttons_use_social_css' ).
					'<td>'.$this->form->get_checkbox( 'buttons_use_social_css' ).$css_min_msg.'</td>',

					$this->form->get_th_html( _x( 'Enqueue the Stylesheet',
						'option label', 'wpsso' ), null, 'buttons_enqueue_social_css' ).
			...
			foreach ( $tabs as $key => $title ) {
				$tabs[$key] = _x( $title, 'metabox tab', 'wpsso' );	// translate the tab title
